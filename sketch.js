 ...
 * David Ionita Revisions:
 * 1. 2022-12-14: added a space gif as the background to play on loop
 * 2. 2022-12-14: double the speed of the balls if you click on the box
 */
...
function preload() {
  ...
  // loads GIF for background
  gif_createImg = loadImage("space.gif");
}
...
function createBox() {
  ...
  fill(255, 102, 153);      // fill() takes R,G,B values as the color
  // draw the text in the box (x,y,width,height) with the color in fill()
  textAlign(CENTER);
  text('Hello World!', BOX_WIDTH/2, BOX_HEIGHT/2+20);   
 
}

function draw() {

  background(gif_createImg);
  createBox();
  ...
  for (var ballNum = 0; ballNum < balls.length; ballNum++) {
    ...
    balls[ballNum].checkForMouseClick();
...
class Ball { // Constructor
  ...
  constructor() {
    ...
    this.mouseClickPrev = mouseClickTimes;
  }
  ...
  checkForMouseClick() {
    if (mouseClickTimes > this.mouseClickPrev) { // checks if mouse has been clicked
      // increases speed accordingly
      this.speedY = this.speedY*mouseClickTimes; 
      this.speedX = this.speedX*mouseClickTimes;
      this.mouseClickPrev = mouseClickTimes; // updates checker value
    }
  }
